"""D03 — the drive and the clutch: sliding carriage, MY-3514C, band, servo."""
import sys, math, os
...
s = Sheet("DRIVE AND CLUTCH — CARRIAGE, MOTOR, BAND, SERVO", "60-09-D03", "AS SHOWN")
s.frame(META)

DRIVE = ["knob_body", "internal_structure", "base_plate", "pad", "halo_diffuser", "led_flex",
         "carriage", "commutation_board", "servo_mount", "collar_2", "bush_2"]
DRIVE += [k for k in engine.parts() if k.startswith(("motor_", "servo_", "mt6701_"))]

vC, hC = engine.radial_section(DRIVE, AZ, hidden=False)

# ---------------------------------------------------------------- SECTION C-C
pC = Port(s, 44.0, 110, 2.0, ox=0.0, oy=0.0)
clC = pC.clip("clipC", 17, 30, 179, 106)
pC.draw(vC, clip=clC, hidden=False, hatch_paths=hC, wv=0.30)
s.rect(17, 30, 179, 106, 0.3)
s.text(17, 27, "SECTION  C–C     scale 2.0 : 1", 3.4, weight="bold")
s.text(17, 141, "On the %g°–%g° axis, clutch ENGAGED. Right of the centreline is the motor at %g°; the servo's inboard "
       "end crosses the centreline." % (AZ, AZ + 180, AZ), 2.2, col="#333")
s.line(pC.px(0), 30, pC.px(0), 136, 0.15, dash="6 2 1 2")
s.text(pC.px(0) + 1.5, 36.5, "CL", 2.2, col="#444")
...
note_list(s, 201, 36, [
 (1, "The drive is friction, not gears: a Ø%s motor bell running on the Ø%s bore through a %s flat silicone band. "
     "Ratio %.2f : 1. v8's gimbal drive and its over-centre clutch are both gone."
     % (n(P.MOTOR_OD, 0), n(2 * P.R_BORE, 0), n(P.MOTOR_BAND_T), RATIO)),
 (2, "JD-Power MY-3514C (2804), Ø%s × %s, on a carriage that slides radially in a stadium hole through the plate. "
     "The carriage shoe is Ø%s on the pad, so the motor's base face is at z %s and its top at z %s."
 ...
 (4, "Commutation: an MT6701 on a %g × %g board inside the carriage, under the motor, reading a Ø6 × 2.5 diametric "
     "magnet in the shaft's lower end through a Ø%s sight hole. Which end of the shaft carries the magnet is "
     "ASSUMED. If it is the top, this board moves and the display rises about 3 mm — over the 40 mm height limit "
     "the specification sets, against %s as modelled." % (12, 12, n(9.0, 0), n(P.HEIGHT, 1))),
...
for i, (eng, lab, x0) in enumerate([(True, "ENGAGED", 300), (False, "RELEASED", 351)]):
    v, h = engine.radial_section(CONTACT, AZ, engaged=eng, hidden=False)
    d = Port(s, x0 + 24, 74, 4.0, ox=56.5, oy=13.4)
...
Z1 = 2.5
PL1 = ["base_plate", "carriage", "commutation_board", "port_face", "usbc_board", "light_board"]
PL1 += [k for k in engine.parts() if k.startswith("mt6701_")]
v1, h1 = engine.plan_section(PL1, Z1)
p1 = Port(s, 77, 206, 0.78)
cl1 = p1.clip("clip1", 17, 150, 120, 112)
p1.draw(v1, clip=cl1, hidden=False, wv=0.25, hatch_paths=h1)
s.rect(17, 150, 120, 112, 0.3)
s.text(17, 147, "PLAN AT z %s — INSIDE THE PLATE     scale %.2f : 1" % (n(Z1, 1), p1.k), 3.0, weight="bold")
...
s.text(17, 269.5, "between them: Ø%s wide, r %s–%s at %g°, with a 10 × 10 tab slot reaching in to r %s."
       % (n(HOLE_D, 1), n(HOLE_R0, 1), n(HOLE_R1, 1), P.MOTOR_AZ, n(TAB_R0, 1)), 2.2)
s.text(17, 273, "The carriage rides on the pad, not on the plate.", 2.2)

# ---------------------------------------------------------------- PLAN AT THE DRIVE BAND
Z2 = (P.Z_DRIVE0 + P.Z_DRIVE1) / 2
PL2 = ["knob_body", "internal_structure", "carriage", "servo_mount", "driver_board", "speaker_cradle", "speaker"]
PL2 += [k for k in engine.parts() if k.startswith(("motor_", "servo_"))]
v2, h2 = engine.plan_section(PL2, Z2)
p2 = Port(s, 199, 206, 0.78)
...
s.text(141, 266, "Contact is a line at %g°, at r %s. Everything else on this level —" % (P.MOTOR_AZ, n(P.R_BORE)), 2.2)
s.text(141, 269.5, "driver board at %g°, speaker, servo tray — stays outside the bell." % engine.model.DRIVER_AZ, 2.2)
...
vI = engine.iso(ISO, direction=(0.9, -1.0, 0.7), hidden=True)
cx, cy = box_centre(vI)
...
save(s, "the60_v9_D03_drive_clutch")
