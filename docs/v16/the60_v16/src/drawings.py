"""v16 general arrangement: plan slices at four heights plus sections through the motor and the back."""
import os, math, sys
...
COL = {"base_plate": "0.45", "rim_ring_STEEL": "0.3", "closing_plate_AL": "0.6", "internal_structure": "#7a6a4a", "halo_diffuser": "#3a9ab0", "knob_body": "0.15", "connect_bracket": "#9a5a1a", "servo_mount": "#9a5a1a",
       "carriage": "#c46a10", "servo_mount": "#5a3a9a", "speaker_cradle": "#c46a10", "blower_saddle": "#c46a10", "hood_lid": "#c46a10", "encoder_shim": "#7a6a4a", "port_face": "#1a7a3c", "led_flex": "#c8892a", "pad": "0.7", "bond_tape": "#b07030"}
def colour(n):
    ...
    if n.startswith("blower") or n.startswith("fan_lead") or n.startswith("hood_"): return "#0a5aa0"
    if "plug" in n or "ribbon" in n or "housing" in n: return "#b02020"
    if n == "speaker": return "#c46a10"
    if n.startswith("adapter") or n.startswith("audio") or n.startswith("bob_") or n.startswith("drv_") or n.startswith("mcu_") or n.startswith("converter"): return "#2a8a6a"
    return "#1a7a3c"
...
fig.suptitle(f"the 60 — v16 general arrangement (Ø185.4): Pi 5 in the plate window, breakouts on studs, MCU tray over the audio board, blower, adapter on the Pi, panel on the seat; V-groove bearings on the bore ridge, the sprung block at 270.  "
...
fig.text(0.01, 0.005, "Bought parts drawn from their own STEP bodies (panel, Pi 5, motor, servo, bearings, speaker, encoder, LRA, sockets, chips, LEDs).  Envelope-only: cooler heatsink, adapter/audio/breakout/converter/touch boards' parts, plugs, ribbon, flex.  "
...
fig.savefig(os.path.join(OUT, "v16_ga.png"), dpi=110)
print("wrote", os.path.join(OUT, "v16_ga.png"))
