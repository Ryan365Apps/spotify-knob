...
WALL_MIN = 1.60
INSERT_M25_D, INSERT_M25_L = 3.5, 4.0                  # PUBLISHED (generic M2.5 heat-set insert: Ø3.5 hole, 4.0 long). v16b (Ryan, 8 Sep): every M3 became M2.5 - one screw size with the Pi's
CSK_M25_D, CSK_M25_HEAD_D, CSK_M25_HEAD_H = 2.7, 4.7, 1.0   # DIN 7991 M2.5 countersunk: Ø2.7 clearance, head Ø4.7, the 90 deg cone 1.0 tall
TAP_M25_D            = 2.05                            # M2.5 tap drill
PAN_M25_HEAD_D, PAN_M25_HEAD_H = 5.0, 2.0              # DIN 7985 M2.5 pan head (the two bond screws)
STAR_M25_OD, STAR_M25_ID = 5.5, 2.7                    # DIN 6798 A M2.5 external-tooth star washer
...
PILLAR_AZ            = [160, 240, 328]                 # M2.5 pillars on the wall for the plate screws from below (v16: 320 -> 328, out of the level shifter's way; v12: 120 moved to 160, out of the speaker's way)
...
RIM_STEP_W, RIM_STEP_Z = 3.0, 5.0                      # the steel rim ring's inward flange (r 72.2-75.2, z 5-8) sits in a rebate on the aluminium core; 7 x M2.5 csk from below through the core's shoulder (v14: the shoulder is 5 tall so the intake grooves can be 3.5 deep through it and leave 1.5)
...
PILLAR_LAND_D        = 9.0                             # the structure's three M2.5 plate screws (r 80.8, az 160/240/328) come up through the ring's inner land: a Ø9 land is left standing in the groove round each so the countersunk head bears on metal
...
RING_SCREW_AZ, RING_SCREW_R = [26.0, 78.75, 135, 191.25, 247.5, 303.75, 334.0], R_CORE_DUCT + 1.5   # 77.3: 7 x M2.5 csk from below through the core's shoulder into the ring's flange, on lands between the passages; the two beside the port slot at 26 / 334 (v15 22.5 / 337.5: the 56-wide slot's walls are at t +-28, the head Ø6.5 broke into them; the M2.5 head Ø4.7 at 22.5 would still reach t 27.2, 0.8 into the wall)
