...
PRINT = {
    ...
    "internal_structure": (lambda s: Pos(0, 0, -Z_PLATE_TOP) * s, "0.20", "upright on the wall foot. SUPPORTS in two places only: under the seat flange (r 62-70.5 at z 20.4 above the bed - a ring inside the wall, hidden face) and under the diffuser lip (r 73.2-76 at z 5.5 - a thin ring outside). Everything else self-supporting: open-topped windows, 1 mm slit roofs, 45 deg hole ends"),
    ...
    "port_face":          (lambda s: Pos(0, 0, PORT_FACE_R0 + PORT_FACE_T) * Rot(0, 90, 0) * s, "0.16", "lying on its outer face (the socket openings print as vertical holes); the USB-C shelf, the light-sensor foot and the two ear arms stand up from it; no supports"),
