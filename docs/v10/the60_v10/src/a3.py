...
SHEETS = [
    ("knob_body", "Knob", "PETG (prototype) / CNC 6082-T6, hard anodised (production)", "print top face down, 0.16 mm; no supports",
     ["outside Ø154.0, bore Ø145.0 (0.45 to the panel's chin corners)", "lip inner edge Ø128.4, lip 3.0 thick, top at z 39.18", "outer chamfer 2.5 x 45° smooth; inner chamfer 1.0; skirt chamfer 1.2",
      f"V-groove 90°, root r {R_GROOVE_ROOT}, z {Z_GROOVE0:.1f}-{Z_GROOVE1:.1f}", f"code-band recess 0.15 deep, z {Z_CODE0:.1f}-{Z_CODE1:.1f}", f"diamond knurl {KNURL_N} starts x 2, {KNURL_DEPTH} deep, helix {KNURL_HELIX_DEG:.1f}°, 4 whole rows z {KNURL_Z0:.1f}-{KNURL_Z1:.1f}; skirt bottom z {Z_SKIRT_BOT}", "wall under the knurl root 3.5"]),
    ...
    ("halo_diffuser", "Halo diffuser", "opal PMMA (production) / natural PETG (prototype)", "print on its underside, 0.20 mm",
     [f"r {R_DIFF_IN}-{R_DIFF_OUT_BOT} at the bottom, to r {R_DIFF_OUT_TOP} at the top; z {HALO_Z0}-{HALO_Z1}", f"{DIFF_CHAMFER} chamfer on the outside top edge", "retained by the structure's lip; sits on the plate's rim outside the LED strip"]),
    ...
    ("base_plate", "Base plate (steel)", "mild steel 8 mm, laser/waterjet cut, powder-coated", "not printed (print a stand-in if the steel is not back)",
     [f"Ø{2*PLATE_R:.0f} x {PLATE_T}; engine-turned edge: {ET_BLOCKS} plain blocks {ET_BLOCK_DEG:.0f}° wide at 0/30/60..., {len(et_groove_azs())} square grooves {ET_GROOVE_W} x {ET_GROOVE_D} at {ET_PITCH} pitch between them, cut in plan; {PLATE_CHAMFER} chamfer top and bottom outer edges", f"Pi window {PI_L + PI_OVERHANG + 1.5:.1f} x {PI_W + 1.5:.1f} with plug notches; carriage stadium hole; port slot {PORT_W} wide from r {PORT_NOTCH_R0}",
