...
SHEETS = [
    ("knob_body", "Knob", "PETG (prototype) / CNC 6082-T6, hard anodised (production)", "print top face down, 0.16 mm; no supports",
     ["outside Ø154.0, bore Ø145.0 (0.45 to the panel's chin corners)", "lip inner edge Ø128.4, lip 3.0 thick, top at z 39.18", "outer chamfer 2.5 x 45° smooth; inner chamfer 1.0; skirt chamfer 1.2",
      f"V-groove 90°, root r {R_GROOVE_ROOT}, z {Z_GROOVE0:.1f}-{Z_GROOVE1:.1f}", f"code-band recess 0.15 deep, z {Z_CODE0:.1f}-{Z_CODE1:.1f}", f"diamond knurl {KNURL_N} starts x 2, {KNURL_DEPTH} deep, helix {KNURL_HELIX_DEG:.1f}°, 4 whole rows z {KNURL_Z0:.1f}-{KNURL_Z1:.1f}", "wall under the knurl root 3.5"]),
    ("internal_structure", "Internal structure", "PETG (prototype) / PA12 (production)", "print upright on the wall foot, 0.20 mm; support the seat flange underside and the wheel windows",
     [f"wall r {R_WALL_IN}-{R_WALL_OUT}, z {Z_PLATE_TOP}-{Z_SEAT_TOP}", f"seat flange r {R_SEAT_IN}-{R_WALL_OUT}, z {Z_SEAT_BOT}-{Z_SEAT_TOP}; three Ø2 locating nubs at {NUB_AZ}°", f"diffuser lip r {R_WALL_OUT}-{R_LIP_OUT}, z {Z_LIP0}-{Z_LIP1}",
      f"wheel posts Ø{WHEEL_POST_D} at r {BUSH_R:.2f}, az {WHEEL_AZ}, to z {Z_POST_TOP:.1f}; bush bores Ø{BUSH_D + 0.1}; wall windows 15.2 x {Z_SEAT_BOT - Z_POST_TOP + 0.3:.1f}", f"pillars Ø7 at r {PILLAR_R}, az {PILLAR_AZ}, to z {Z_DECK0:.2f}; M3 inserts top (all) and bottom ({PLATE_SCREW_AZ})",
      f"encoder tower at {ENC_AZ}°, window through the wall at the code band; LRA pad at {LRA_AZ}°", f"flex slot 32 x 2.5 through the seat at 0°, offset -4; {len(port_azs())} ports Ø{PORT_D} at z {PORT_Z}", "motor relief at 90°"]),
    ("deck", "Mezzanine deck", "PETG", "print upside down (legs up), 0.20 mm; no supports",
     [f"disc Ø140 x 1.5, z {Z_DECK0:.2f}-{Z_DECK1:.2f}", f"three legs Ø5.5 down to the Pi's board top (z {PI_Z_TOP}), M2.5 inserts", f"tab slot 6.0 x 10.6 at 90°; ribbon slot 22 x 3 at ({HDMI0_X:.1f}, {PI_Y0 - 6.5:.1f})",
    ...
    ("halo_diffuser", "Halo diffuser", "opal PMMA (production) / natural PETG (prototype)", "print on its underside, 0.20 mm",
     [f"r {R_DIFF_IN}-{R_DIFF_OUT_BOT} at the bottom, to r {R_DIFF_OUT_TOP} at the top; z {HALO_Z0}-{HALO_Z1}", f"{DIFF_CHAMFER} chamfer on the outside top edge", "retained by the structure's lip; sits on the plate's rim outside the LED flex"]),
    ("carriage", "Motor carriage", "PETG", "print standing on the shoe, 0.16 mm; brace the 28 mm tab",
    ...
    ("servo_mount", "Servo tray (on the deck)", "PETG", "print flat, 0.20 mm",
     [f"tray for the AGFRC envelope {SERVO_L} x {SERVO_W} x {SERVO_H}; walls 1.0, floor {SERVO_TRAY_T}", "open toward the carriage tab; lead exit at the inner end", "2 x M2.5 into deck inserts"]),
    ("speaker_cradle", "Speaker cradle", "PETG", "print flat, 0.20 mm",
    ...
    ("port_face", "Rear port face", "PETG", "print standing, 0.16 mm",
     [f"{PORT_W} wide x {PORT_FACE_T} thick at r {PORT_FACE_R0}, z {-PAD_T}-{Z_PLATE_TOP}", f"barrel nose Ø6.2 at t {PORT_BARREL_T}, z {BARREL_AXIS_Z}; USB-C opening at t {PORT_USBC_T}, z {USBC_AXIS_Z}; jack Ø6.3 at t {PORT_JACK_T}, z {JACK_AXIS_Z}; light aperture Ø{SENSOR_HOLE_D} at t {PORT_LIGHT_T}",
      "shelf for the USB-C board; foot for the light-sensor board; two ears at t ±16 on the plate top (M2)"]),
    ...
    ("bush_0", "Eccentric bush (x3)", "PETG (prototype) / brass (production)", "print pin up, 0.12 mm",
     [f"Ø{BUSH_D} shank x 6, Ø{BUSH_FLANGE_D} x {BUSH_FLANGE_T} flange, Ø{WHEEL_BORE} pin x {WHEEL_W} offset {WHEEL_ECC}; 2 mm hex socket"]),
    ("base_plate", "Base plate (steel)", "mild steel 8 mm, laser/waterjet cut, powder-coated", "not printed (print a stand-in if the steel is not back)",
     [f"Ø{2*PLATE_R:.0f} x {PLATE_T}", f"Pi window {PI_L + PI_OVERHANG + 1.5:.1f} x {PI_W + 1.5:.1f} with plug notches; carriage stadium hole; port slot {PORT_W} wide from r {PORT_NOTCH_R0}",
      f"3 x M3 countersunk from below at r {PILLAR_R}, az {PLATE_SCREW_AZ}; 2 x M3 csk for the speaker cradle; ground-bond hole; 2 x Ø1.6 for the port-face ears"]),
]
def sheet(pdf, i, name, title, material, printnote, dims):
    ...
    fr.text(233, 46, "the 60 — v10", fontsize=12, weight="bold"); fr.text(300, 46, f"{title}", fontsize=12, weight="bold")
...
pdf_path = os.path.join(OUT, "the60_v10_A3_drawings.pdf")
with PdfPages(pdf_path) as pdf:
    # sheet 0: assumptions
    fig = plt.figure(figsize=(420/25.4, 297/25.4)); fr = fig.add_axes([0, 0, 1, 1]); fr.axis("off"); fr.set_xlim(0, 420); fr.set_ylim(0, 297)
    fr.add_patch(plt.Rectangle((10, 10), 400, 277, fill=False, lw=1.2))
    fr.text(14, 278, "the 60 — v10 — sheet 0: the ASSUMED numbers (every one of these must be verified on the real part before the drawing is trusted)", fontsize=12, weight="bold")
    ...
    fr.text(14, 60, "Build-time changes against V10-SPECIFICATION.md are listed in V10.md.  Sheets 1-11 follow: one per made part.", fontsize=8.5)
    pdf.savefig(fig); plt.close(fig)
