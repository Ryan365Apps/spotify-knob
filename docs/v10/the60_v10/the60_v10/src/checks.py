...
def allowed(a, b):
    ...
    rules = [
        ...
        (lambda: "base_plate" in s and (has("internal_structure", "speaker_cradle", "port_face", "jack_board", "led_flex", "halo_diffuser") or anyp("led_")), "stands on the plate"),
        ...
        (lambda: anyp("led_") and "led_flex" in s, "LEDs on the flex"),
        ...
        (lambda: any("ENVELOPE" in x and x.startswith("motor") for x in s) and has("internal_structure", "led_flex"), "ENVELOPE: the Ø35 envelope's lowest 4.3 mm stands where the stator base is ASSUMED Ø30"),
        # display stack
...
moving = lambda k: (k in ("carriage", "commutation_board", "internal_structure", "base_plate", "servo_mount", "knob_body", "halo_diffuser", "led_flex", "pad", "deck", "motion_board")
                    or k.startswith("motor") or k.startswith("servo") or k.startswith("mt6701") or k.startswith("led_") or k.startswith("pi_") or k.startswith("hdmi") or k.startswith("header") or k.startswith("cooler"))
...
ports = port_azs()
ok(f"{len(ports)} of {PORT_N} perimeter ports cut, Ø{PORT_D} at z {PORT_Z}: {ports}")
ok(f"port area {len(ports)*math.pi*(PORT_D/2)**2:.0f} mm2; rim gap {2*math.pi*LENS_R*RIM_GAP:.0f} mm2 — passive convection only (no fan)")
