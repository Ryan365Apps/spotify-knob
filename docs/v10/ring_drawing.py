"""the 60 v10 - top ring drawing (Ø154, plain bore). Not CAD; numbers from V10-SPECIFICATION.md section 8.1."""
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
from matplotlib.patches import Circle, Polygon, Rectangle

R_KNOB, CH_OUT, CH_IN = 77.0, 2.5, 1.0
R_PIC, R_LIP = 63.5, 64.2
R_LENS, R_GLASS, R_CHIN, R_BORE = 70.0, 66.1, 72.05, 72.5
flat = R_KNOB - CH_OUT - (R_LIP + CH_IN)
metal = R_KNOB - R_LIP
total = R_KNOB - R_PIC

fig = plt.figure(figsize=(19.2, 10.8), dpi=100)
fig.suptitle("the 60 — v10 top face: the metal ring with the 5-inch panel at Ø154 (drawing, not CAD)", fontsize=16, weight="bold", y=0.975)

ax = fig.add_axes([0.03, 0.08, 0.5, 0.84]); ax.set_aspect("equal"); ax.axis("off")
ax.set_xlim(-95, 95); ax.set_ylim(-95, 95)
...
for r, ls, c, lab in [(R_LENS, "--", "#2f8fe0", "lens Ø140 (ASSUMED)"), (R_GLASS, ":", "#777", "panel glass Ø132.2"), (R_BORE, "-", "#999", "knob bore Ø145")]:
...
ax.text(0, 88, f"metal ring, picture edge to knob edge: {total:.1f} mm total   (v9: 18.7 mm)", ha="center", fontsize=13, weight="bold")
ax.annotate("outer chamfer 2.5 (bright, diamond-cut)", xy=(R_KNOB - 1.2, 20), xytext=(40, 84), fontsize=10, arrowprops=dict(arrowstyle="->", lw=0.8))
ax.annotate("inner chamfer 1.0 (smooth); lip edge Ø128.4, 0.7 outside the picture", xy=(-64.7, 5), xytext=(-92, 80), fontsize=10, arrowprops=dict(arrowstyle="->", lw=0.8))
ax.annotate(f"flat ring Ø{2*(R_LIP+CH_IN):.1f} – Ø{2*(R_KNOB-CH_OUT):.1f}  =  {flat:.1f} mm wide", xy=(-70, -15), xytext=(-92, -84), fontsize=10, arrowprops=dict(arrowstyle="->", lw=0.8))
ax.annotate("hidden under the lip: lens Ø140 (dashed), panel glass Ø132.2 (dotted), bore Ø145 (grey), chin (red)", xy=(50, -49), xytext=(-40, -92), fontsize=10, arrowprops=dict(arrowstyle="->", lw=0.8))
...
bx.set_xlim(60, 82); bx.set_ylim(20.5, 43)
...
bx.add_patch(Rectangle((62.0, z_seat0), 72.1 - 62.0, z_seat1 - z_seat0, fc="#8a7a5a", ec="k", lw=0.6))  # structure seat flange
bx.add_patch(Rectangle((70.5, 25), 72.1 - 70.5, z_seat0 - 25, fc="#8a7a5a", ec="k", lw=0.6))  # wall
...
bx.text(74.6, 30, "knob wall 4.5", fontsize=8, ha="center", va="center", rotation=90)
...
fig.text(0.56, 0.12, f"Read this as: seen from above the ring is {metal:.1f} mm of metal — {flat:.1f} flat, 2.5 bright outer chamfer, 1.0 small inner chamfer — and {total:.1f} mm from the picture edge to the knob edge.\n"
         "The lip overlaps the lens by 5.8 mm and reaches to 0.7 mm outside the picture. v9 had 18.7 mm of metal (picture Ø87.6 in a Ø125 knob).\n"
         "The ring cannot be narrower without a smaller knob (question 1: Ø150 needs an undercut) or a wider outer chamfer; the panel's chin (red) is what sets the bore.",
         fontsize=11, ha="left", va="top", wrap=True)
fig.savefig("v10_top_ring.png")
