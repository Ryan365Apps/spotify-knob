...
def base_plate():
    ...
    for sy in (-24.0, 24.0):                                   # speaker cradle, csk from below
        p -= Pos(SPEAKER_CENTRE[0], sy + SPEAKER_CENTRE[1], 0) * (cyl(1.7, -1, PLATE_T+1) + Pos(0,0,-0.01)*Cone(3.25, 1.7, 1.6, align=(Align.CENTER, Align.CENTER, Align.MIN)))
...
def motor_relief(engaged=True):
    ...
    return k
LEDGE_PASS_R = 51.3   # head pass-through hole in the ledge, 1.7 inboard of the M4 hole so the Ø7 head clears the wall
def structure():
    ...
    for az in COLUMN_AZ:                                                      # display columns: 4 x 6 stadium section, fully rounded ends
        s += blk(COLUMN_R1 - COLUMN_R0 - COLUMN_W, COLUMN_W, Z_LEDGE_TOP - 0.01, Z_SEAT0 + 0.01, az, (COLUMN_R0 + COLUMN_R1)/2)
        s += zbore(COLUMN_W, Z_LEDGE_TOP - 0.01, Z_SEAT0 + 0.01, az, COLUMN_R0 + COLUMN_W/2)
        s += zbore(COLUMN_W, Z_LEDGE_TOP - 0.01, Z_SEAT0 + 0.01, az, COLUMN_R1 - COLUMN_W/2)
    # encoder tower
    ...
    for az in DISC_MOUNT_AZ_DEV:                                              # M4 clearance through the seat tabs
        ...
        s -= zbore(7.6, Z_LEDGE_BOT - 1, Z_LEDGE_TOP + 1, az, LEDGE_PASS_R)      # the screw head passes up through the ledge here
...
def seat_tabs():
    """four 1.2 mm seat tabs in the gap between the board top and the disc back: one plain
    stadium-shaped plate each, from the column's centre to the M4 hole (part of the structure)"""
    t = None
    for az_col, az_hole in zip(COLUMN_AZ, DISC_MOUNT_AZ_DEV):
        ...
        ac = D(az_col); rc = (COLUMN_R0 + COLUMN_R1)/2
        cx, cy = rc * math.cos(ac), rc * math.sin(ac)
        mx, my = (hx + cx)/2, (hy + cy)/2
        L = math.hypot(hx - cx, hy - cy)
        ang = math.degrees(math.atan2(hy - cy, hx - cx))
        zc = (Z_SEAT0 + Z_SEAT1)/2
        plate = Pos(mx, my, zc) * Rot(0, 0, ang) * Box(L, 8.0, SEAT_T)
        plate += Pos(hx, hy, Z_SEAT0) * Cylinder(4.0, SEAT_T, align=(Align.CENTER, Align.CENTER, Align.MIN))
        plate += Pos(cx, cy, Z_SEAT0) * Cylinder(4.0, SEAT_T, align=(Align.CENTER, Align.CENTER, Align.MIN))
        t = plate if t is None else t + plate
    t = t & tube(R_DISC - 0.3, R_LEDGE_IN, Z_SEAT0 - 1, Z_SEAT1 + 1)     # never outside the disc
...
def halo_diffuser():
    ...
    return revolve_profile([(R_DIFF_IN, HALO_Z0), (R_DIFF_OUT_BOT, HALO_Z0),
                            (R_DIFF_OUT_TOP - c * lean, HALO_Z1 - c),        # 0.5 chamfer on the outside top edge
                            (R_DIFF_OUT_TOP - c, HALO_Z1), (R_DIFF_IN, HALO_Z1)])
def led_flex():
...
def servo_mount():
    ...
    L = SERVO_L + 1.5
    m = Box(L, SERVO_W + 2.0, 1.5, align=(Align.MIN, Align.CENTER, Align.MIN))
    m += Pos(0, 0, 1.5) * Box(L, SERVO_W + 2.0, SERVO_H + 0.5, align=(Align.MIN, Align.CENTER, Align.MIN))
...
def speaker_cradle():
    ...
    c = tube(SPEAKER_D/2 + 2.3, 18.3, 0, 4.45)
    for i in range(3):
        f = Rot(0, 0, 120*i + 90) * (Pos(21.25, 0, 4.44) * Box(3.1, 3.0, 2.8, align=(Align.CENTER, Align.CENTER, Align.MIN))
                                     + Pos(19.75, 0, 6.05) * Box(1.0, 3.0, 1.2, align=(Align.CENTER, Align.CENTER, Align.MIN)))
        c += f
    for sy in (-24.0, 24.0):                                                   # ears on the tangential axis, away from the wall
        c += Pos(0, sy, 0) * Cylinder(4.0, 2.0, align=(Align.CENTER, Align.CENTER, Align.MIN))
        c += Pos(0, sy * 22/24, 0) * Box(6.0, 6.0, 2.0, align=(Align.CENTER, Align.CENTER, Align.MIN))
        c -= Pos(0, sy, -1) * Cylinder(1.7, 5, align=(Align.CENTER, Align.CENTER, Align.MIN))
