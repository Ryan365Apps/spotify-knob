...
BUSH_FLANGE_T        = 0.60
WHEEL_AZ             = [33, 153, 273]                  # board-edge rule; rotated 10 deg on 4 Sep review so no post sits over the speaker or beside a display column
...
LRA_AZ               = 258.0                           # RULING: wall beside a wheel post (280); uncramped side of the base (4 Sep review)
SPEAKER_D, SPEAKER_T = 40.0, 9.05                      # Soberton, vendor mesh
SPEAKER_CENTRE       = (-31.5, 0.0)                    # at 180 deg (4 Sep review): nothing above it, the post at 270 is over the driver board instead
...
COLUMN_AZ            = [52, 142, 218, 308]             # 7 deg off the M4 holes, on the side away from the wheel posts the M4 holes: the column never covers the hole, and the far-side columns clear the board edge
...
DIFF_CHAMFER         = 0.50                            # outside top edge (4 Sep review)
R_LIP_OUT            = 60.50
PLATE_R              = 62.00
PAD_R                = 61.00
...
KNURL_ON             = True
KNURL_N, KNURL_DEPTH = 56, 1.00
KNURL_ROWS           = 4                               # whole rows of diamonds between the lands (4 Sep review: pattern must fit the band exactly)
KNURL_ROOT_OFF, KNURL_BAND_OV, KNURL_LAND = 0.50, 0.30, 0.80
KNURL_BANDS          = KNURL_ROWS                      # one cutter band per row, so the band seams fall on the row lines
CH_TOP, CH_IN, CH_BOT = 2.50, 1.00, 1.20
KNURL_Z0             = Z_SKIRT_BOT + CH_BOT + KNURL_LAND          # 11.0  a valley line: the bottom row is whole
KNURL_Z1             = Z_KNOB_TOP - CH_TOP - KNURL_LAND           # 34.8  a valley line: the top row is whole
KNURL_PITCH          = _m.pi * 2 * R_KNOB / KNURL_N               # 7.01 circumferential
KNURL_ROW_H          = (KNURL_Z1 - KNURL_Z0) / KNURL_ROWS         # 5.95 axial period of the rows
KNURL_HELIX_DEG      = _m.degrees(_m.atan((KNURL_PITCH / 2) / KNURL_ROW_H))   # 30.5: the helix angle that makes the rows fit
