"""Diamond knurl on the v9 knob body's side.
...
OUT = M.OUT
KEY = (KNURL_N, KNURL_DEPTH, KNURL_HELIX_DEG, KNURL_ROOT_OFF, KNURL_BANDS, KNURL_BAND_OV,
...
def _knurl_star(r, za, zb, z_ref, phase_ref, sign, ext=1.0):
    d, tip = KNURL_DEPTH, 0.5
...
def knurl_band(r, z0, z1, z_ref, ov):
    d, out = KNURL_DEPTH, 1.5
    za, zb = z0 - ov, z1 + ov
    shell = _band_shell(r, za, zb, d, out)
    sp = _knurl_star(r, za, zb, z_ref, 0.0, +1)
    sm = _knurl_star(r, za, zb, z_ref, math.pi / KNURL_N, -1)
    ...
    if i == 0:
        ...
        if 1 <= i <= nb:
            ...
            for attempt in range(3):
                q = p - knurl_band(R_KNOB, za, zb, Z_SKIRT_BOT, ov)
                if q.is_valid and len(q.solids()) == 1:
                    p = q; break
                ov += 0.011
            else:
                raise RuntimeError(f"band {i} never produced a valid single solid")
        elif i == nb + 1:
            try: p = p.clean()
