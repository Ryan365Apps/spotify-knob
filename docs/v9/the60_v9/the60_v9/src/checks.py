...
# every M4 seat hole is clear of the board in plan
for az in DISC_MOUNT_AZ_DEV:
    a = math.radians(az); hx, hy = DISC_MOUNT_PCD/2*math.cos(a), DISC_MOUNT_PCD/2*math.sin(a)
    probe = Pos(hx, hy, Z_LEDGE_TOP) * Cylinder(2.15, Z_SEAT0 - Z_LEDGE_TOP, align=(Align.CENTER, Align.CENTER, Align.MIN))
    v = inter(probe, bought_on["display_pcb"])
    (ok if v < 0.05 else fail)(f"M4 screw path at {az} deg clear of the board ({v:.2f} mm3)")
