"""v11 general arrangement: plan slices at four heights plus sections through the motor and the back."""
import os, math, sys
...
allp = {**made, **bought}
COL = {"base_plate": "0.45", "rim_ring_STEEL": "0.3", "closing_plate_AL": "0.6", "internal_structure": "#7a6a4a", "halo_diffuser": "#3a9ab0", "knob_body": "0.15", "deck": "#9a5a1a",
...
fig.suptitle(f"the 60 — v11 general arrangement.  Pi 5 on the pad through the plate window, mezzanine deck, 5-inch panel bonded to the seat, 8 mm plate.  "
...
ax = fig.add_axes([0.34, 0.50, 0.32, 0.45]); plan(ax, 12.0, "plan at z = 12 — motor on its carriage, Pi with its heatsink, speaker, pillars, LRA, wall ports", floor + ["knob_body"])
ax = fig.add_axes([0.67, 0.50, 0.32, 0.45]); plan(ax, 25.0, "plan at z = 25 — deck boards: adapter, audio, motion, converter, servo tray, tall tab; wheels", floor + ["knob_body"])
ax = fig.add_axes([0.01, 0.03, 0.32, 0.45]); plan(ax, 31.5, "plan at z = 31.5 — the panel on its seat, chin at 180°, flex at 0°", floor + ["knob_body", "panel_glass", "panel_components_ENVELOPE"])
def section(ax, az, title):
    ...
    for z, t in ((0, "plate underside z 0"), (Z_PLATE_TOP, "plate top / halo bottom 8.0"), (Z_MOTOR_TOP, "motor top 18.2"),
                 (Z_DECK0, f"deck {Z_DECK0:.1f}"), (Z_GLASS0, f"glass back {Z_GLASS0:.1f}"), (Z_KNOB_TOP, f"knob top {Z_KNOB_TOP:.1f}")):
        ax.plot([-83, 83], [z, z], color="0.85", lw=0.5, ls=":"); ax.annotate(t, (-83, z + 0.3), fontsize=6.5, color="0.5")
ax = fig.add_axes([0.34, 0.03, 0.32, 0.45]); section(ax, 90, "section through the motor (azimuth 90°, right = motor side; servo on the deck above the Pi)")
...
fig.text(0.01, 0.005, "Bought parts drawn from their own STEP bodies (panel, Pi 5, motor, servo, bearings, speaker, encoder, LRA, sockets, chips, LEDs).  Envelope-only: cooler heatsink, adapter/audio/motion/converter/touch boards' parts, plugs, ribbon, flex.  "
         "Colours: grey plate, brown structure, orange deck, black knob, crimson Pi, gold motor, purple servo/wheels, teal boards, blue display, red plugs and cables.", fontsize=7.5, color="0.35")
fig.savefig(os.path.join(OUT, "v11_ga.png"), dpi=110)
print("wrote", os.path.join(OUT, "v11_ga.png"))
