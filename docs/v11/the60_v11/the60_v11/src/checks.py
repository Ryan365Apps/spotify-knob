"""v11 automated checks: solidity, pairwise interference across EVERY body (made and bought)
...
def allowed(a, b):
    ...
    rules = [
        ...
        (lambda: has("base_plate", "rim_ring_STEEL", "closing_plate_AL") and has("internal_structure", "speaker_cradle", "port_face", "jack_board", "halo_diffuser", "led_strip_ENVELOPE", "converter_gap_pad_ASSUMED"), "stands on the plate"),
...
moving = lambda k: (k in ("carriage", "commutation_board", "internal_structure", "base_plate", "rim_ring_STEEL", "closing_plate_AL", "servo_mount", "knob_body", "halo_diffuser", "led_strip_ENVELOPE", "pad", "deck", "motion_board")
...
n_air = len(air.solids())
(ok if n_air == 1 else fail)(f"the air path (intake grooves, collector, {2 * sum(1 for k in range(20) if (PI_Y0 + PI_W + 0.75 + DUCT_MARGIN + FIN_CH_W/2 + 0.5 + FIN_PITCH * k) + FIN_CH_W/2 <= R_CORE_DUCT - 1.0)} fin channels, plenum, exhaust grooves) is {n_air} connected volume, {air.volume/1000:.1f} cm3")
ok(f"intake: {len(intake_azs())} grooves {INTAKE_W} x {INTAKE_H} = {len(intake_azs()) * INTAKE_W * INTAKE_H:.0f} mm2 behind a {INTAKE_UNDERCUT_H} undercut over az {INTAKE_AZ0:.0f}-{INTAKE_AZ1:.0f} ({2*math.pi*PLATE_R*(INTAKE_AZ1-INTAKE_AZ0)/360*INTAKE_UNDERCUT_H:.0f} mm2 mouth); exhaust: {len(EXHAUST_AZ)} grooves = {len(EXHAUST_AZ) * INTAKE_W * INTAKE_H:.0f} mm2, downward through the pad's voids at az {EXHAUST_VOID}")
ok(f"blower envelope {BLOWER_L} x {BLOWER_W} x {BLOWER_H} reserved at az {BLOWER_AZ:.0f}, r {BLOWER_R:.0f} (under the speaker; 2 mm into the web) - part not fitted")
