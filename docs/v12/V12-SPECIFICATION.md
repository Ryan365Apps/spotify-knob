# the 60 — version 12 specification

**Status: as-built specification for the v12 CAD.** This document describes every component and mechanism the v12 model contains, says which numbers are measured and which are assumed, grades every requirement, and separates what is true of the v10/v11 prototype only (Raspberry Pi 5) from what carries to production.

Revision: 6 September 2026 (evening) — v12 = v11 (the Ø175.4 knob with its 20 mm rim, the heatsink plate) plus **Ryan's deck-less layout**: no mezzanine deck, every board on the plate, the display adapter's driver board on the Pi's standoffs, the code band and the wheels dropped to the bell band's limit, the seat 2.4 lower. Two facts from the adapter's datasheet (fetched this revision) are folded in: its holes are the Pi's 58 × 49 pattern, and the adapter is three parts (driver board, 150 mm flat cable, display connect board) — v10 and v11 wrongly had the panel flex reaching the driver board. Superseded text has been removed, not struck through. The v11 documents stay in `docs/v11/` as delivered.
...
**Finding 3 (v10) — what still holds.** The Pi's tallest features are its stacked USB-A sockets at 16.2 above the board (MEASURED) and standing the Pi through a window in the plate is what removes height. **The cooler's fan is not fitted**; in v12 the adapter's underside is 1.2 mm over the heatsink's fins, so a fan has no room at all — the first print measures the passive Pi (section 6.9). **What sets v12's height is the wheels**: their groove must sit above the code band, the code band above the bell band's contact zone on the bore, and that zone starts at the knob skirt's bottom (14.9, over the halo's lip). With the band narrowed to 2.0 of contact the seat lands at 28.0 and the knob top at 36.4 — 2.8 below v11. The adapter's HDMI socket (23.7) would allow 26.0; nothing else comes close (section 8.3).
...
| Height | derived: **37.9** v12 with the pad (knob top 36.4; v11 40.7); **≈ 34** production with the CM5; the stack and what decides it are in section 8 | report | brief |
...
| Wheels z | 23.4–27.4 | **21.5–25.5**, groove 21.9–25.1 (0.3 above the code band), collar tops 0.5 under the seat flange |
| Code band | 18.4–22.6 | **17.4–21.6** on the bore, straight above the bell band's contact zone (the band 9.0–16.9 on the bell, met by the bore over 14.9–16.9); encoder tower at **310°** to z 24.0, its board's leads out of a notch under the sensor |
...
**The aluminium core**, r < 75.2, 6082, **black hard anodised on every face including the internal ones** (rule 1: the cavity's only route to the cold plate is radiation), with two masked bare pads — round the ground-bond screw at (50, −20) and under the converter (rule 2). In section: a **4 mm web** (z 4–8), a **3 mm duct** (z 1–4) and a **1 mm closing plate** flush in a recess (z 0–1), screwed on with six M2.5 countersunk screws that all land in solid web, so it comes off to clean the channels (rule 5). A rebate r 72.2–75.2, z 4–8 takes the ring's flange; seven M3 countersunk screws from below through the core's shoulder hold the ring. Through it: the Pi window (with notches for the USB-C plug's head, the HDMI plug and the USB-A plug), the carriage hole, the port slot from r 55 — each through-cut with a 2.5 mm solid wall round it so the duct does not leak into it. Into its top face: **16 M2.5 tapped blind holes 3.0 deep** (converter 2, audio 4, motion 4, servo frame 2, connect bracket 2, speaker cradle 2) and an M3 for the ground bond at (40, 12) — they stop 1.0 above the duct, so nothing above the web needs an island; only the six closing screws from below have Ø7 islands. On its top face, **two ribs 2 × 6 mm** at r 56–70 (az 158 and 212 — the plate top is nearly full; v11 had ten), inside r 70 so the structure still enters from below. Mass ≈ 139 g.

**The duct** (rules 7–12): air enters through a **1.2 mm undercut on the bottom outer edge** outside the pad (r 84.7–87.7) over the front and sides, az 45–315 (496 mm² of mouth; the rear is closed); through **21 radial grooves 6 × 2** in the ring's underside (252 mm²; the pad closes them from below; the two grooves that would land on the motor's through-hole and the one at the USB-C plug's notch are omitted); into a **collector** r 66–70.2 round the front and sides; through **19 straight fin channels 3 wide × 3 tall on a 4 mm pitch** — twelve on the +y side and seven on the −y side of the off-centre Pi window, front to rear; into a **rear plenum** either side of the port slot (r 55–70.2, az 22–52 and 308–338); and out through **six exhaust grooves** at az 27–41 and 319–333 (72 mm²) that open **downward where the pad is cut away** (az 15–46 and 314–345) — rear only, a different arc from the intake, not the socket slot (rule 10). **Two circuits in v12**: the Pi's USB-C plug notch reaches r 70.6 at az 231 and its wall severs the front collector, so the −y side (7 channels, 6 intake grooves at 242–298°, the 308–338 plenum, the exhaust at 319–333) is its own intake-to-exhaust circuit of 9.3 cm³ and the +y side (12 channels, 14 grooves, the 22–52 plenum, the exhaust at 27–41) is 15.4 cm³ — the checker requires each to have both. No opening through the top face or the edge face (rules 8, 9). A **blower envelope 28 × 18 × 5** is reserved in the 308–338 plenum at az 328, r 63 (where the air leaves) — 3 in the duct, 2 into the web; the part is **not fitted** (rule 12). What the geometry would not give: a 30 × 30 spot — the motor's through-hole and the Pi window rule it out everywhere — so the envelope suits a slim or 20 × 20 blower; and a single circuit, because the plug notch is fed mainly by its own three intake grooves.
...
One part, as v9, scaled to the bore: a wall r 81.2–82.8 from the plate top to the seat; the **seat flange** at the top (r 62–82.8, z 26.0–28.0 — a 21 mm shelf; the glass bonds at r 62.5–65.6 as before) with the 32 × 2.5 flex slot at 0° and three Ø2 locating nubs at 60/210/330° — the display module bonds to it; three wheel posts Ø8 at r 76.9, az 30/150/270 (bush sockets open to the plate side), webbed to the wall, with a 15.2-wide window through the wall above each for the wheel and **the seat flange cut away entirely above each wheel** (print review, 5 Sep: nothing needed it, and it was a shelf hanging over the window — the glass bridges the three notches, the tape ring is unbroken); the encoder tower at 310° to z 24.0, its sensor window through the wall open to the top, its board slot open right through the seat with a 0.25 crush rib (the 8 × 10 board, z 15.9–25.9, is pressed in from the seat side before the display goes on and the lens caps it) and, v12, a notch through the tower's inner face under the sensor for the board's leads (the gap between the tower's top and the seat flange is too small to solder in); the motor's window in the wall open up to the seat flange (no roof to bridge); the LRA pad at 258°; the 1.1 mm band the LED strip sticks to (r 82.8–83.9, z 8–13.5) and the diffuser-retaining lip above it (r to 86.7, 13.5–14.3); **five Ø7 pillars at r 77.2, az 50/120/180/240/320**, webbed to the wall, with M3 inserts at the top for the deck's screws and, at 120/240/320, at the bottom for the plate's screws; 89 vertical vent slits 1.0 × 3.5 at z 14.7–18.2 on a 2.5° grid; the motor relief at 90°; the three blind plate-screw insert holes end in 45° points. No ledge, no display columns, no seat tabs, no carrier, and **no lid** (thermal plan rule 14): the deck's boards reach z 29.4 and the panel's back components start at 29.4, so there is no room for one in v11 — the bonded glass and the seat flange close the cavity; the lid arrives in production with the boards on the plate. **Printing**: upright on the wall's foot; supports only under the seat flange (a hidden ring inside the wall) and under the diffuser lip (a thin ring outside) — everything else is open-topped, a 1 mm bridge or steeper than 45° (`src/printcheck.py`).
...
### 8.3 v12 stack as built — one level, adapter on the Pi, display bonded to the seat
...
| 4.2–18.2 | motor on its carriage; band on the bell 9.0–16.9, met by the bore over 14.9–16.9 | v9 case B |
...
| 14.78–16.38 | adapter on 12 mm standoffs; its parts to 18.9, flex connector 18.4, buttons 20.5, **HDMI socket to 23.68**; touch board on it to 22.4; flat cable to 20.8 | the Pi's thickness + the socket |
| 17.4–21.6 code band; 21.5–25.5 wheels; **26.0–28.0 seat flange** | the wheels' groove 0.3 above the code band, the code band 0.5 above the bell band, the band 2.0 above the skirt's bottom | **the deciding chain** |
| 28.0–28.5 | bonding tape under the glass edge | +0.5 |
| 28.5–30.48 | panel glass 1.98 (its back components hang 1.5 below it inside r 34) | |
| 30.48–32.98 | lens 2.5 (ASSUMED) | |
| 33.38–36.38 | lip 3.0 after the 0.4 rim gap | |
| **37.9** | **overall with the pad** (v11: 40.7) | |

What each part costs against the wheels' floor: nothing under the seat reaches it — the adapter's socket is 2.3 short of the seat's underside, so flipping the adapter, deleting the touch board or shortening the standoffs saves 0. The next 2.0 would come from the bell band (a 1.0 contact instead of 2.0 — not recommended) or from moving the encoder above the wheels (a turret above the seat at 310°, its top 0.4 under the rotating crown — costed, not built; question 19). The bonding tape **+0.5**; the 8 mm plate and the 5.5 halo band **0** in height.

### 8.4 Option B — cooler deleted, Pi inverted, SoC on a boss to the plate

Pi inverted with its USB-A and Ethernet stacks standing on the pad through the plate window: component face at z 16.2, board 16.2–17.48, its back-side parts to 18.93; SoC face at 13.86; boss 5.4 tall on the 8 mm plate, 0.5 pad. Everything above is as built: the seat is set by the wheels, so **37.9 with the pad**, the same as v12.
