"""v12 general arrangement: plan slices at four heights plus sections through the motor and the back."""
import os, math, sys
...
fig.suptitle(f"the 60 — v12 general arrangement.  Pi 5 in the plate window on the closing plate, every board on the plate, adapter on the Pi's standoffs, 5-inch panel bonded to the seat.  "
...
ax = fig.add_axes([0.01, 0.03, 0.32, 0.45]); plan(ax, 29.0, "plan at z = 29 — the panel on its seat, chin at 180°, flex at 0°", floor + ["knob_body", "panel_glass", "panel_components_ENVELOPE"])
...
fig.savefig(os.path.join(OUT, "v12_ga.png"), dpi=110)
print("wrote", os.path.join(OUT, "v12_ga.png"))
