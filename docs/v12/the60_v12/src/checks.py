"""v12 automated checks: solidity, pairwise interference across EVERY body (made and bought)
...
def allowed(a, b):
    ...
    rules = [
        ...
        (lambda: any(x.startswith("encoder_") and x != "encoder_board" for x in s) and has("encoder_board", "internal_structure"), "encoder on its board in its seat"),
...
display = ("panel_glass", "lens_ASSUMED", "panel_components_ENVELOPE", "bond_tape", "knob_body", "internal_structure", "halo_diffuser", "encoder_board")
tallest = []
for n, s in {**made, **bought_on}.items():
    if n in display or n.startswith("panel_flex") or n.startswith("touch_tail") or n.startswith("collar") or n.startswith("bush") or n.startswith("bearing") or n.startswith("nub"): continue
...
ok(f"seat floor: wheels' groove over the code band needs {Z_CODE1 + 0.3 + 6.1:.1f}; the adapter's HDMI socket ({Z_ADAPTER_HDMI_TOP:.2f}) needs {Z_ADAPTER_HDMI_TOP + 0.3 + 2.0:.1f}; seat top {Z_SEAT_TOP}")
...
ok(f"drive band on the bore z {max(Z_DRIVE0, Z_SKIRT_BOT):.1f}-{Z_DRIVE1:.1f}; code band {Z_CODE0:.1f}-{Z_CODE1:.1f}; groove {Z_GROOVE0:.1f}-{Z_GROOVE1:.1f}; seat {Z_SEAT_TOP}; glass {Z_GLASS0:.2f}-{Z_GLASS1:.2f}; lens top {Z_LENS1:.2f}; knob top {Z_KNOB_TOP:.2f} (v11 39.18)")
...
for dz in (12.0, 8.0, 5.0, 3.0, 1.0):
    ...
    v = inter(b, made["internal_structure"])
    if v > 0.25: blocked.append(f"at {dz:+.0f}: {v:.1f} mm3")                   # the crush rib's 0.05 interference is allowed
(ok if not blocked else fail)("the encoder board drops into the tower's open-topped slot from above: " + ("clear" if not blocked else "; ".join(blocked)))
rise = Z_GLASS1 - (ENC_BOARD_Z0 + ENC_BOARD_H)                  # how far the board can lift before it meets the lens (it sits under the lens overhang, r 66.1-70)
engage = ENC_TOWER_TOP - (ENC_BOARD_Z0 - 0.4) - rise
(ok if engage >= 2.0 and ENC_BOARD_Z0 + ENC_BOARD_H <= Z_SEAT_TOP - 0.5 else fail)(f"encoder board {ENC_BOARD_Z0}-{ENC_BOARD_Z0 + ENC_BOARD_H:.1f} (seat top {Z_SEAT_TOP}); capped by the lens: can lift {rise:.1f}, still {engage:.1f} deep in the tower's {ENC_TOWER_TOP - (ENC_BOARD_Z0 - 0.4):.1f} slot; leads out of the notch under the sensor")
...
ok(f"intake: {len(intake_azs())} grooves (v11: 22; one more dropped at the USB-C plug's notch) {INTAKE_W} x {INTAKE_H} = {len(intake_azs()) * INTAKE_W * INTAKE_H:.0f} mm2 behind a {INTAKE_UNDERCUT_H} undercut over az {INTAKE_AZ0:.0f}-{INTAKE_AZ1:.0f} ({2*math.pi*PLATE_R*(INTAKE_AZ1-INTAKE_AZ0)/360*INTAKE_UNDERCUT_H:.0f} mm2 mouth); exhaust: {len(EXHAUST_AZ)} grooves = {len(EXHAUST_AZ) * INTAKE_W * INTAKE_H:.0f} mm2, downward through the pad's voids at az {EXHAUST_VOID}")
