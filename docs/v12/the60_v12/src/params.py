"""the 60 (Cadrane) — v12 parameter block.  Every dimension lives here.

v12 (2026-09-06): v11's Ø175.4 knob and heatsink plate with Ryan's deck-less layout - every board on the plate,
the adapter driver board on standoffs over the Pi, the code band and the wheels dropped, the seat 2.4 lower.
Built from docs/v12/V12-SPECIFICATION.md.  Every number is tagged in the comment beside it:
...
Z_DRIVE0, Z_DRIVE1   = Z_BELL_BOT + 0.5, Z_SKIRT_BOT + 2.0   # 9.0 - 16.9 band on the bell (v11: to 17.8): the bore meets it over its lowest 2.0 (the skirt's bottom is 14.9, over the halo lip), and the code band sits straight above it - v12: the band narrowed so the code band and the wheels could drop. The 3.6 Ryan saw under the strip was this contact zone (14.9-18.5)
...
Z_CODE0, Z_CODE1     = Z_DRIVE1 + 0.5, Z_DRIVE1 + 0.5 + 4.2   # 17.4 - 21.6 code band (AEDR-8300 is 3.96 tall); v11: 18.5-22.7
ENC_TOWER_TOP        = Z_CODE1 + 2.4                   # 24.0: the tower's top (PRINT: 2.0 of material over the sensor pocket, a 6 mm bridge)
...
Z_SEAT_TOP           = Z_CODE1 + 0.3 + 6.1             # 28.0: the structure's seat (display bonded straight onto it). v12: set by the wheels - their groove must clear the code band (21.6 + 0.3, then 1.6 to the ridge, 2.0 to the wheel's top, 0.5 to the flange, 2.0 flange). The tallest thing under the seat, the adapter's HDMI socket (23.7), would allow 26.0. v11: 30.4
...
Z_WHEEL1             = Z_SEAT_TOP - 2.0 - 0.5          # 25.5 (collar top 0.5 under the seat's underside 26.0)
Z_WHEEL0             = Z_WHEEL1 - WHEEL_W              # 21.5
Z_RIDGE_MID          = (Z_WHEEL0 + Z_WHEEL1) / 2       # 23.5
Z_GROOVE0            = Z_RIDGE_MID - WHEEL_V_H - WHEEL_V_FLAT/2   # 21.9 (0.3 above the code band)
Z_GROOVE1            = Z_RIDGE_MID + WHEEL_V_H + WHEEL_V_FLAT/2   # 25.1
Z_POST_TOP           = Z_WHEEL0 - BUSH_FLANGE_T        # 20.9
ENC_RIB_H, ENC_RIB_W = 0.25, 0.6                       # ASSUMED crush rib in the board slot (slot 1.2, board 1.0: 0.05 interference at the rib)
ENC_BOARD_H          = 10.0                            # ASSUMED 8 x 10 board from 1.5 under the code band to 1.5 under the seat's top: dropped through the seat's slot into the tower's open-topped slot (before the display goes on); the lens caps it. v12: its leads leave at the BOTTOM through a notch in the tower's inner face (the 2.4 between the tower top and the seat flange is too little to solder on)
ENC_BOARD_Z0         = Z_CODE0 - 1.5                   # 15.9 (board to 25.9, 2.1 under the seat's top)
Z_SEAT_BOT           = Z_SEAT_TOP - 2.0                # 26.0 seat flange
...
BLOWER_AZ, BLOWER_R, BLOWER_L, BLOWER_W, BLOWER_H = 328.0, 63.0, 18.0, 28.0, 5.0   # rule 12: envelope for a radial blower, part NOT fitted; 3 in the duct + 2 into the web. v12: in the 308-338 rear plenum, where the channels end and the exhaust grooves start (the 277 spot is now the USB-C plug's notch); still 28 x 18, no 30 x 30 spot exists
...
CLOSING_SCREW_XY     = [(-60.0, 0.0), (-56.0, 16.0), (-56.0, -16.0), (44.0, 10.0), (44.0, -6.0), (52.0, 27.0)]   # 6 x M2.5 csk from below: the front crescent beyond the Pi window (solid web, no channels), the rear wedge inside the port slot's walls, and one island in the 22-52 plenum
...
SLIT_Z0, SLIT_Z1     = Z_CODE1 + 0.3, Z_SEAT_BOT - 1.0   # 21.9 - 25.0: above the code band, under the seat (v11: 14.7-18.2, which is now the band)
...
BLIND_D              = 3.0                             # tapped blind holes from the plate top stop 1.0 above the duct (web 4.0)
...
CB_SCREW_XY          = [(46.0, 17.0), (46.0, -14.0)]
...
AUDIO_L, AUDIO_W, AUDIO_H = 40.0, 25.0, 6.0            # Ryan's 40 x 25 (x by y here); ASSUMED height incl. the board
...
MOTION_L, MOTION_W, MOTION_H = 30.0, 30.0, 7.4         # connectors 5.0 tall
MOTION_AZ, MOTION_R  = 50.0, 52.0                      # v12: on the plate, r 37-67 at 50 deg (Ryan's picture had 42), over the 22-52 plenum; 0.7 off the Pi's USB-A shells, 2 off the connect bracket's ear
...
GND_BOND_XY          = (40.0, 12.0)                    # chassis ground: M3 tapped blind from the top, masked bare pad; in solid web inside the port slot's wall
