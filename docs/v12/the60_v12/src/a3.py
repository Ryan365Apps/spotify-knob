...
SHEETS = [
    ...
    ("internal_structure", "Internal structure", "PETG (prototype) / PA12 (production)", "print upright on the wall foot, 0.20 mm; supports ONLY under the seat flange (inside) and the diffuser lip (outside)",
     [f"wall r {R_WALL_IN}-{R_WALL_OUT}, z {Z_PLATE_TOP}-{Z_SEAT_TOP}", f"seat flange r {R_SEAT_IN}-{R_WALL_OUT}, z {Z_SEAT_BOT}-{Z_SEAT_TOP}; three Ø2 locating nubs at {NUB_AZ}°", f"LED-strip band r {R_WALL_OUT}-{R_STRIP_BACK}, z {Z_PLATE_TOP}-{HALO_Z1} (strip r {R_STRIP_IN}-{R_STRIP_OUT}); diffuser lip r {R_WALL_OUT}-{R_LIP_OUT}, z {Z_LIP0}-{Z_LIP1}",
      ...
      f"encoder tower at {ENC_AZ}° to z {ENC_TOWER_TOP:.1f}, board slot open to the top, lead notch through its inner face at z {ENC_BOARD_Z0 - 0.4:.1f}-{Z_CODE0 - 0.6:.1f}, window through the wall at the code band; motor window open to the seat; LRA pad at {LRA_AZ}°", f"flex slot 32 x 2.5 through the seat at 0°, offset -4; {len(slit_azs())} vent slits {SLIT_W} x {SLIT_Z1 - SLIT_Z0:.1f} at z {SLIT_Z0}-{SLIT_Z1}, 2.5° grid", "motor relief at 90°"]),
    ...
    ("carriage", "Motor carriage", "PETG", "print standing on the shoe, 0.16 mm; no supports (12 mm bridge over the sensor pocket); 4+ perimeters for the tab",
     [f"shoe Ø37 x {CARRIAGE_T}; locating rim Ø35.6/37 x 0.6", "commutation-board pocket 12.4 x 12.4 x 3.3; sight hole Ø9", f"push tab {CARRIAGE_TAB_T} x {CARRIAGE_TAB_W} to z {CARRIAGE_TAB_TOP:.1f} (v11: 28.5 through the deck; the servo is now beside it on the plate)", f"travel {CLUTCH_LIFT} radial in the plate's stadium hole"]),
...
def sheet(pdf, i, name, title, material, printnote, dims):
    ...
    fr.text(233, 46, "the 60 — v12", fontsize=12, weight="bold"); fr.text(300, 46, f"{title}", fontsize=12, weight="bold")
...
pdf_path = os.path.join(OUT, "the60_v12_A3_drawings.pdf")
with PdfPages(pdf_path) as pdf:
    # sheet 0: assumptions
    fig = plt.figure(figsize=(420/25.4, 297/25.4)); fr = fig.add_axes([0, 0, 1, 1]); fr.axis("off"); fr.set_xlim(0, 420); fr.set_ylim(0, 297)
    fr.add_patch(plt.Rectangle((10, 10), 400, 277, fill=False, lw=1.2))
    fr.text(14, 278, "the 60 — v12 — sheet 0: the ASSUMED numbers (every one of these must be verified on the real part before the drawing is trusted)", fontsize=12, weight="bold")
    fr.text(14, 270, f"{DATE}.  Stack: plate {PLATE_T}, adapter {Z_ADAPTER0:.2f}-{Z_ADAPTER1:.2f} (socket to {Z_ADAPTER_HDMI_TOP:.2f}), code band {Z_CODE0:.1f}-{Z_CODE1:.1f}, seat {Z_SEAT_TOP}, glass {Z_GLASS0:.2f}-{Z_GLASS1:.2f}, lens to {Z_LENS1:.2f}, knob top {Z_KNOB_TOP:.2f}, overall {HEIGHT:.2f} with the pad.  Ø{2*R_KNOB:.0f}.", fontsize=8.5)
