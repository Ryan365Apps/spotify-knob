...
ENC_BOSS_H           = ENC_BOARD_Z0 - Z_SEAT_TOP       # 0.90
...
PLATE_CHAMFER        = 1.0                             # top and bottom outer edges; the top one runs r 86.7-87.7, outside the diffuser's seat (r 85.8-86.7)
MOTOR_CLEAR          = 2.0
...
INTAKE_UNDERCUT_H, INTAKE_UNDERCUT_R = 1.5, PAD_R      # a 1.5 undercut on the bottom outer edge outside the pad, r 84.7-87.7 (v14: 620 mm2 of mouth; design-changes item 5 asks 500-700)
INTAKE_N, INTAKE_W, INTAKE_H = 24, 7.0, 3.5            # 24 positions, 21 cut: radial grooves 7 wide x 3.5 deep in the rim's underside (the pad closes them) and through the core's shoulder into the collector - 514 mm2 of throat (v13: 6 x 2, 252). 7 wide leaves 0.55 each side of the ring screws' countersinks on the lands between grooves
EXHAUST_AZ           = [28.0, 36.0, 44.0, 316.0, 324.0, 332.0]   # rule 10: rear only, downward, through a void in the pad, on a different arc from the intake; the same 7 x 3.5 section (147 mm2 - the exhaust is now the restriction, deliberately kept to the rear arcs)
...
CLOSING_SCREW_XY     = [(-60.0, 0.0), (-56.0, 16.0), (-56.0, -16.0), (44.0, 10.0), (44.0, -6.0), (56.0, 31.0)]   # 6 x M2.5 csk from below: the front crescent beyond the Pi window (solid web, no channels), the rear wedge inside the port slot's walls, and one island in the 22-52 plenum
...
RING_BOND_AZ, RING_BOND_R = 56.25, 73.7                # the rim ring's dedicated bond screw (GROUNDING 4.1): M3 from the top through the ring's flange into the core's shoulder, external-tooth star washer under the head, both faces bare; on the land between the intake grooves at 50.6 and 61.9 (the rear arc's lands round the port slot are all taken by the exhaust grooves and the ring screws), 35 mm from the chassis bond
BLEED_AZ, BLEED_R    = 200.0, 71.5                     # the knob's 1 MOhm bleed (GROUNDING 5): a phosphor-bronze leaf on a boss on the seat flange, bearing lightly UP on the crown's underside at r 71.5 - the smallest radius the knob offers (the lens ends at 70, the code ring starts at 72.5); between the wheel notch at 150 and the nub at 210
