...
for n, note in (("base_plate", "base_plate_AL: CNC 6082 aluminium core, external faces black hard anodised, internal faces chromate/bare (ducted underside, ribs on top)"), ("rim_ring_STEEL", "rim_ring_STEEL: machined steel ring, plain edge, intake/exhaust grooves underneath"), ("closing_plate_AL", "closing_plate_AL: 1 mm laser-cut aluminium")):
...
FUSE = [("pi5_board", lambda n: n.startswith("pi_")), ("mt6701", lambda n: n.startswith("mt6701")),
        ...
        ("usbc_shell_wire", lambda n: n.startswith("usbc_shell_wire")), ("motor_bond_wire", lambda n: n.startswith("motor_bond_wire")), ("sensor_cable", lambda n: n.startswith("sensor_cable"))]
def fuse_all(shapes):
    out = shapes[0]
    for s in shapes[1:]:
        try: out = out + s
...
assembly(True, "the60_v14_assembly")
assembly(False, "the60_v14_assembly_free_spin")
with open(os.path.join(pd, "README.txt"), "w") as f:
    f.write("the 60 - v14 - printable parts (print orientation; they will not assemble as exported)\n\n")
    ...
    f.write(f"rim_ring_STEEL        machined steel ring r {RIM_IN:.1f}-{PLATE_R:.1f} x {PLATE_T:.0f}: plain edge, 1.0 chamfers, inward flange onto the core, {len(intake_azs())} intake + {len(EXHAUST_AZ)} exhaust grooves in its underside, {INTAKE_UNDERCUT_H} intake undercut front and sides\n")
