...
SHEETS = [
    ...
    ("internal_structure", "Internal structure", "PETG (prototype) / PA12 (production)", "print upright on the wall foot, 0.20 mm; supports ONLY under the seat flange (inside) and the diffuser lip (outside)",
     [f"wall r {R_WALL_IN}-{R_WALL_OUT}, z {Z_PLATE_TOP}-{Z_SEAT_TOP}", f"seat flange r {R_SEAT_IN}-{R_WALL_OUT}, z {Z_SEAT_BOT}-{Z_SEAT_TOP}; three Ø2 locating nubs at {NUB_AZ}°", f"LED-strip band r {R_WALL_OUT}-{R_STRIP_BACK}, z {Z_PLATE_TOP}-{HALO_Z1} (strip r {R_STRIP_IN}-{R_STRIP_OUT}, {LED_N} LEDs, {HALO_PEAK_A:.2f} A full white); diffuser lip r {R_WALL_OUT}-{R_LIP_OUT}, z {Z_LIP0}-{Z_LIP1}",
      ...
      f"encoder boss 8.4 x 12.4 x {ENC_BOSS_H:.1f} on the seat flange's top at {ENC_AZ}°, r {ENC_R:.0f}, two M2 inserts (the 8 x 12 board lies on it, sensor up, {ENC_GAP} under the crown's code ring); no tower, no window in the wall; motor window open to the seat; LRA pad at {LRA_AZ}° with the pogo-board clip beside its flex tail at {LRA_CLIP_AZ}° (two ribs r 76-81.7, a 0.4 groove each, open-topped); the knob-bleed boss 5 x 8 x 1 on the flange at {BLEED_AZ}°, r {BLEED_FOOT_R}, M2 insert", f"flex slot 32 x 2.5 through the seat at 0°, offset -4; {len(slit_azs())} vent slits {SLIT_W} x {SLIT_Z1 - SLIT_Z0:.1f} at z {SLIT_Z0}-{SLIT_Z1}, 2.5° grid", "motor relief at 90°"]),
    ...
    ("base_plate", "Base plate - aluminium core (the heatsink)", "6082 aluminium, CNC two-sided; EXTERNAL faces black hard anodised, INTERNAL faces chromate or bare - no masked pads (design-changes item 2)", "machined, not printed (print a PETG stand-in without the duct for the fit check)",
     [f"r < {RIM_IN:.1f} x {PLATE_T}: {WEB_T:.0f} web (z {WEB_T:.0f}-{PLATE_T:.0f}), {DUCT_H:.0f} duct (z {CLOSING_T:.0f}-{CLOSING_T + DUCT_H:.0f}) closed by a 1 mm plate in a recess; rebate r {R_CORE_DUCT:.1f}-{RIM_IN:.1f}, z {RIM_STEP_Z:.0f}-{PLATE_T:.0f} for the ring's flange on a {RIM_STEP_Z:.0f} shoulder, 7 x M3 csk from below at r {RING_SCREW_R}; the ring's bond screw M3 tapped 3.0 into the shoulder at az {RING_BOND_AZ}; the CHASSIS bond M3 tapped blind at {GND_BOND_XY} (the only chassis bond)",
      ...
      f"Pi window (Pi at ({PI_X0:.0f}, {PI_Y0:.0f}), with the USB-C plug head, HDMI plug and USB-A plug notches), carriage hole, port slot; {len(top_screw_xy())} x M2.5 tapped blind {BLIND_D} deep from the top (converter, audio, motion, servo frame, connect bracket, speaker cradle) and THE chassis bond (M3) at {GND_BOND_XY}; {len(CLOSING_SCREW_XY)} x M2.5 tapped from below for the closing plate (Ø7 islands in the duct)",
      f"{len(RIB_AZ)} ribs {RIB_T} x {RIB_H} tall on the top face at r {RIB_R0:.0f}-{RIB_R1:.0f}, az {RIB_AZ}"]),
    ("rim_ring_STEEL", "Rim ring (steel)", "mild steel, machined; finish OPEN (it decides whether the bond spot is masked or the star washer cuts through)", "machined, not printed",
     [f"r {RIM_IN:.1f}-{PLATE_R:.1f} x {PLATE_T}; PLAIN edge face (v14: the engine turning is removed; the treatment is chosen separately); {PLATE_CHAMFER} chamfer top and bottom outer edges",
      ...
      f"intake: {INTAKE_UNDERCUT_H} undercut on the bottom outer edge outside r {INTAKE_UNDERCUT_R:.1f} over az {INTAKE_AZ0:.0f}-{INTAKE_AZ1:.0f} ({2*3.14159*PLATE_R*(INTAKE_AZ1-INTAKE_AZ0)/360*INTAKE_UNDERCUT_H:.0f} mm2), feeding {len(intake_azs())} radial grooves {INTAKE_W} x {INTAKE_H} in the underside ({len(intake_azs())*INTAKE_W*INTAKE_H:.0f} mm2; the pad closes them) - all parametric (INTAKE_N/W/H/AZ0/AZ1); exhaust: {len(EXHAUST_AZ)} grooves at az {EXHAUST_AZ}, opening downward where the pad is cut away",
    ...
    ("closing_plate_AL", "Closing plate (aluminium)", "1 mm aluminium, laser-cut, black anodised", "laser-cut, not printed",
     [f"disc r {R_CORE_DUCT - 0.1:.1f} x {CLOSING_T}, flush in the core's recess; same Pi window, carriage hole and port slot as the core", f"four 9.5 square lands reaching into the Pi window at the Pi's holes {[(round(x, 1), round(y, 1)) for x, y in pi_holes_xy()]}, Ø2.7 csk from below: the Pi and adapter stack screws to them (M2.5 x 6 into the standoffs)", f"{len(CLOSING_SCREW_XY)} x M2.5 countersunk; unscrew to clean the fin channels"]),
]
def sheet(pdf, i, name, title, material, printnote, dims):
    ...
    fr.text(233, 46, "the 60 — v14", fontsize=12, weight="bold"); fr.text(300, 46, f"{title}", fontsize=12, weight="bold")
...
pdf_path = os.path.join(OUT, "the60_v14_A3_drawings.pdf")
with PdfPages(pdf_path) as pdf:
    # sheet 0: assumptions
    fig = plt.figure(figsize=(420/25.4, 297/25.4)); fr = fig.add_axes([0, 0, 1, 1]); fr.axis("off"); fr.set_xlim(0, 420); fr.set_ylim(0, 297)
    fr.add_patch(plt.Rectangle((10, 10), 400, 277, fill=False, lw=1.2))
    fr.text(14, 278, "the 60 — v14 — sheet 0: the ASSUMED numbers (every one of these must be verified on the real part before the drawing is trusted)", fontsize=12, weight="bold")
