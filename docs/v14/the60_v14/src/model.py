...
def base_plate():
    ...
    p -= zbore(2.5, RIM_STEP_Z - 3.0, RIM_STEP_Z + 0.1, RING_BOND_AZ, RING_BOND_R)         # v14: the ring's bond screw, M3 tapped 3.0 into the shoulder from its top face (GROUNDING 4.1)
    ...
    return p
def rim_ring():
    """the steel ring: plain edge, chamfers, the flange onto the core, intake undercut and grooves, exhaust grooves, the fixing holes, the bond screw's hole"""
    p = cyl(PLATE_R, 0, PLATE_T)                                                        # v14: a plain edge face (the engine turning is gone; the replacement treatment is chosen separately)
    c = PLATE_CHAMFER
    ...
    p -= zbore(3.4, RIM_STEP_Z - 1, PLATE_T + 1, RING_BOND_AZ, RING_BOND_R)                # v14: the dedicated bond screw's clearance hole through the flange (GROUNDING 4.1)
    return p
def closing_plate():
...
def structure():
    ...
    # v13: the encoder's boss on the seat flange's top face - the sensor faces up at the code ring on the crown's underside
    s += blk(8.4, 12.4, Z_SEAT_TOP - 0.01, ENC_BOARD_Z0, ENC_AZ, ENC_R)
    for t in (-4.75, 4.75):
        s -= zbore(INSERT_M2_D, ENC_BOARD_Z0 - INSERT_M2_L, ENC_BOARD_Z0 + 1, ENC_AZ, ENC_R, t=t)   # M2 inserts (through the 2.0 flange + 0.9 boss)
    ...
    # v14: the knob's bleed contact - a boss on the seat flange's top with an M2 insert (GROUNDING 5)
    s += blk(5.0, 8.0, Z_SEAT_TOP - 0.01, Z_SEAT_TOP + 1.0, BLEED_AZ, BLEED_FOOT_R)
    s -= zbore(INSERT_M2_D, Z_SEAT_TOP + 1.0 - INSERT_M2_L, Z_SEAT_TOP + 2, BLEED_AZ, BLEED_FOOT_R)
...
def motion_parts():
    ...
    out["motion_connectors_ENVELOPE"] = P * Pos(-11, 0, 0) * Box(5.0, 24.0, CONN_H, align=(Align.CENTER, Align.CENTER, Align.MIN))   # Pico-Lock right-angle on the inner (motor) edge, 2.0 mated (v13 allowed 5.0)
    out["motion_usb_plug_ENVELOPE"] = P * Pos(0, -11, 0) * Box(10.0, 8.0, USB_PLUG_H, align=(Align.CENTER, Align.CENTER, Align.MIN))   # its pre-made USB lead's moulded plug on the 0-deg-facing edge, 4.0
    return out
def converter_body():
...
def wire_bodies():
    ...
    out["motor_bond_wire_3"] = box_at(-0.65, 20.0, 45.5, 46.8, Z_PLATE_TOP + 3.3, Z_PLATE_TOP + 4.6)
    return out
def sensor_cable_bodies():
    ...
    out["sensor_cable_3"] = box_at(xc - 0.75, 22.0, 32.0, 46.5, Z_PLATE_TOP + 8.5, Z_PLATE_TOP + 9.5)             # over to the motion board (drawn as a slab: the loop and the run)
    out["sensor_cable_4"] = box_at(20.0, 22.0, 32.0, 34.0, Z_PLATE_TOP + BOARD_STANDOFF + 1.6 + CONN_H - 0.01, Z_PLATE_TOP + 9.5)   # down into the motion board's connector
    return out
