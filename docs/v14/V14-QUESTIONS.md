...
| 23 | Rim edge treatment | **open** (design-changes item 5): the engine turning is removed in v14 and the edge face is plain; the vent openings are parametric so the chosen treatment swaps in without a rebuild |
| 24 | Rim ring finish | **open** (`GROUNDING.md`): decides whether the ring's bond-screw spot is masked or the star washer cuts through |
| 25 | Bleed leaf | form built as a leaf on the seat flange at 200° bearing on the crown at r 71.5 (ASSUMED 4 × 0.2 phosphor bronze); bench: its drag must not be felt, and the contact resistance path through 1 MΩ |
