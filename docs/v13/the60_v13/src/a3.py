...
def sheet(pdf, i, name, title, material, printnote, dims):
    ...
    fr.text(233, 46, "the 60 — v13", fontsize=12, weight="bold"); fr.text(300, 46, f"{title}", fontsize=12, weight="bold")
...
pdf_path = os.path.join(OUT, "the60_v13_A3_drawings.pdf")
with PdfPages(pdf_path) as pdf:
    # sheet 0: assumptions
    fig = plt.figure(figsize=(420/25.4, 297/25.4)); fr = fig.add_axes([0, 0, 1, 1]); fr.axis("off"); fr.set_xlim(0, 420); fr.set_ylim(0, 297)
    fr.add_patch(plt.Rectangle((10, 10), 400, 277, fill=False, lw=1.2))
    fr.text(14, 278, "the 60 — v13 — sheet 0: the ASSUMED numbers (every one of these must be verified on the real part before the drawing is trusted)", fontsize=12, weight="bold")
