...
for n, note in (("base_plate", "base_plate_AL: CNC 6082 aluminium core, external faces black hard anodised, internal faces chromate/bare (ducted underside, ribs on top)"), ("rim_ring_STEEL", "rim_ring_STEEL: machined STAINLESS ring, 120 obround openings through the outer wall into the underside groove, reeding, 0.3 polished chamfers, bare brushed"), ("closing_plate_AL", "closing_plate_AL: 1 mm laser-cut aluminium")):
...
GROUPS = [   # (group name, predicate on the body name).  First match wins; unmatched bodies stay top-level.
    ...
    ("board_washers",       lambda n: n.startswith("board_washer") or n.startswith("motion_washer")),
...
assembly(True, "the60_v15_assembly")
assembly(False, "the60_v15_assembly_free_spin")
with open(os.path.join(pd, "README.txt"), "w") as f:
    f.write("the 60 - v15 - printable parts (print orientation; they will not assemble as exported)\n\n")
    for n, (fn, layer, note) in PRINT.items(): f.write(f"{n:20s} {layer} mm  {note}\n")
    f.write(f"base_plate_AL         CNC 6082 aluminium core r < {RIM_IN:.1f}: {WEB_T:.0f} web, {DUCT_H:.0f} duct with {FIN_CH_W:.0f} x {DUCT_H:.0f} fin channels on a {FIN_PITCH:.0f} pitch, ribs {RIB_H:.0f} tall on top; external faces black hard anodised, internal faces chromate or bare - no masked pads\n")
    f.write(f"rim_ring_STEEL        machined STAINLESS ring r {RIM_IN:.1f}-{PLATE_R:.1f} x {PLATE_T:.0f}: {len(vent_azs())} obround openings {VENT_W} x {VENT_H} on 3 deg through the 3.7 outer wall into an underside groove the pad closes, {INTAKE_UNDERCUT_H} undercut under the outer wall, {len(intake_azs())} intake + {len(EXHAUST_AZ)} exhaust passages across the inner land, the blower trench's notch, {REED_N} reeds, 0.3 polished chamfers; bare, brushed axially\n")
