...
def allowed(a, b):
    ...
    rules = [
        ...
        # v15: the blower unit
        (lambda: has("blower_BFB0305HA-C") and has("blower_inlet_gasket_ASSUMED", "blower_saddle") or anyp("blower_screw") and (has("blower_BFB0305HA-C", "blower_saddle", "motion_board", "base_plate", "blower_inlet_gasket_ASSUMED")), "the blower on its gasket under the saddle, the two M2 through the board, saddle, blower and gasket into the piers"),
        ...
        (lambda: anyp("motor_bond_wire") and (anyp("motor_bond_wire") or anyp("motor") or has("carriage", "motion_connectors_ENVELOPE", "motion_board")), "the motor frame's bond lead: under a base screw, up beside the blower, to the motion board's edge connector"),
        (lambda: anyp("sensor_cable") and (anyp("sensor_cable") or has("carriage", "commutation_board", "motion_connectors_ENVELOPE", "servo_mount", "motion_board")), "the rotor sensor lead: out of the carriage's notch, up beside the tab, its loop over the servo frame, up beside the blower into the motion board's edge connector"),
...
tallest = []
for n, s in {**made, **bought_on}.items():
    if n in display or n.startswith("panel_flex") or n.startswith("touch_tail") or n.startswith("collar") or n.startswith("bush") or n.startswith("bearing") or n.startswith("locating_pin") or n.startswith("encoder_") or n.startswith("knob_bleed"): continue
...
for dz in (-40.0, -25.0, -12.0, -4.0, -1.0):
    ...
    if v > 0.05: blocked.append(f"at {dz:+.0f}: {v:.1f} mm3")
(ok if not blocked else fail)("the structure (with its posts, pillars, bosses and lip) enters the knob from below on a straight path: " + ("clear" if not blocked else "; ".join(blocked)))
...
sec("boards on the plate: blind tapped holes stop in the web + pier (v15: 2 mm web, Ø7 piers 2.5 into the duct under every top screw)")
holes = top_screw_xy()
(ok if BLIND_D <= WEB_T + PIER_H - 1.0 else fail)(f"{len(holes)} x M2.5 + 2 x M2 (blower) + 1 x M3 (chassis bond) blind holes {BLIND_D} deep from the top face: {WEB_T} of web + {PIER_H} of pier = {WEB_T + PIER_H} of metal, {WEB_T + PIER_H - BLIND_D:.1f} of floor left, {DUCT_H - PIER_H:.1f} of duct under each pier (rule 5: thickening downward only; {len(CLOSING_SCREW_XY)} closing screws from below have full-depth islands)")
...
sec("mass")
rho = {"base_plate": 2.7, "rim_ring_STEEL": 7.9, "closing_plate_AL": 2.7}
tot = 0
...
(ok if bb.max.Z <= Z_MOTION0 - 1.6 else fail)(f"blower {BLOWER_L} x {BLOWER_W} x {BLOWER_H} at az {BLOWER_AZ}, r {BLOWER_R}, z {bb.min.Z:.1f}-{bb.max.Z:.1f} on its {BLOWER_GASKET_T} gasket; saddle plate to {Z_BLOWER1 + SADDLE_T}; motion board {Z_MOTION0}-{Z_MOTION0 + 1.6}, its connectors to {Z_MOTION0 + 1.6 + CONN_H} ({Z_SEAT_BOT - (Z_MOTION0 + 1.6 + CONN_H):.1f} under the seat flange)")
...
(ok if d_lid >= 0.5 else fail)(f"the hood lid to the structure's wall / post webs: {d_lid:.2f} mm")
...
ok(f"closing plate {CLOSING_T} mm in its recess on a {GASKET_T} gasket, {len(CLOSING_SCREW_XY)} x M2.5 csk (v14: 6) - free span about 30 mm; ribs {len(RIB_AZ)} x {RIB_T} x {RIB_H} tall at r {RIB_R0:.0f}-{RIB_R1:.0f}")
...
ok(f"deflection under knob load: NIL before and after - the knob rides on the wheels on the structure's wall (r {R_WALL_OUT}), the structure stands on the plate at its wall foot and the pillars (r {PILLAR_R}), all inside the solid rim (r >= {RIM_IN}); the web carries the boards and the servo only (item 1's report-back)")
def f1(t, a):
